...
class Withdraw extends StatefulWidget {
  ...
  final List <CurrencyNetwork>? network;
   const Withdraw({super.key, this.index, this.currencyNetwork, this.currency,this.network});
       ...
       child: TradeBitContainer(
         ...
         child: SafeArea(
           child: TradeBitScaffold(
             appBar: PreferredSize(preferredSize: Size.fromHeight(Dimensions.h_55), child: TradeBitAppBar(title: 'Withdraw',onTap: ()=> Navigator.pop(context)
             )
             ),
             body: GetBuilder(
               ...
               builder: (controller) {
                 return widget.currencyNetwork?[widget.index ?? 0].withdrawEnable == true ? SingleChildScrollView(
                   child: Column(
                     children: [
                       TradeBitContainer(
                         ...
                         child: Column(
                           ...
                           children: [
                             ...
                             Column(
                               crossAxisAlignment: CrossAxisAlignment.start,
                               children: [
                                 TradeBitTextWidget(title: 'Destination Address', style: AppTextStyle.themeBoldTextStyle(color: Theme.of(context).highlightColor,fontSize: FontSize.sp_14)),
                                   ...
                                   child: TradeBitContainer(
                                     ...
                                     child: Align(
                                       alignment: Alignment.centerLeft,
                                         child: TradeBitTextWidget(title: " ${controller.token}",style: AppTextStyle.normalTextStyle(FontSize.sp_14, Theme.of(context).shadowColor.withOpacity(0.8),))),
                                   ),
                                 ),
                                 VerticalSpacing(height: Dimensions.h_10),
                                   ...
                                   children: [
                                     Row(
                                       children: [
                                         ...
                                         TradeBitTextWidget(title: widget.network?[0].withdrawCommission ?? '', style: AppTextStyle.themeBoldTextStyle(color: AppColor.green,fontSize: FontSize.sp_14)),
                                       ],
                                     ),
                                     Row(
                                       children: [
                                         ...
                                         TradeBitTextWidget(title: ' ${widget.currencyNetwork?[widget.index ?? 0].quantity} ${widget.currencyNetwork?[widget.index ?? 0].symbol}     ', style: AppTextStyle.themeBoldTextStyle(color: AppColor.appColor,fontSize: FontSize.sp_14)),
                                       ],
                                     ),
                                   ],
                                 ),
                             ...
                             TradeBitTextButton(
                               margin: EdgeInsets.only(right: Dimensions.h_15,bottom: Dimensions.h_10),
                               height: Dimensions.h_40,
                                 labelName: 'Proceed', onTap: () {
                               controller.withDraw(context,widget.currency ?? '');
                 ...
                 ) : Center(child: Text(widget.currencyNetwork?[widget.index ?? 0].withdrawDesc ?? ''));
               },
             ),
           ),
         ),
